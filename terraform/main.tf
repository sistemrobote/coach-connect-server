...
resource "aws_iam_policy" "lambda_dynamodb_access" {
  name        = "AllowLambdaDynamoDBAccess-${var.environment}"
  description = "Allow Lambda to access DynamoDB user_tokens table"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
        ...
        Resource = "arn:aws:dynamodb:${var.aws_region}:${data.aws_caller_identity.current.account_id}:table/user_tokens"
      }
    ]
  })
}
